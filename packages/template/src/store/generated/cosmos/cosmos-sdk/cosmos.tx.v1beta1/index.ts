  ...
  actions: {
    ...
    async ServiceSimulate(
      ...
      try {
        ...
        if (subscribe)
          commit('SUBSCRIBE', {
            action: 'ServiceSimulate',
            payload: { options: { all }, params: { ...key }, query }
          })
        return getters['getSimulate']({ params: { ...key }, query }) ?? {}
      } catch (e) {
        throw new Error(
          'QueryClient:ServiceSimulate API Node Unavailable. Could not perform query: ' +
            e.message
        )
      }
    },
    ...
    async ServiceGetTx(
      ...
      try {
        ...
        throw new Error(
          'QueryClient:ServiceGetTx API Node Unavailable. Could not perform query: ' +
            e.message
        )
      }
    },

    async ServiceBroadcastTx(
      { commit, rootGetters, getters },
      {
        options: { subscribe, all } = { subscribe: false, all: false },
        params,
      ...
      try {
        ...
        if (subscribe)
          commit('SUBSCRIBE', {
            action: 'ServiceBroadcastTx',
            payload: { options: { all }, params: { ...key }, query }
          })
        return getters['getBroadcastTx']({ params: { ...key }, query }) ?? {}
      } catch (e) {
        throw new Error(
          'QueryClient:ServiceBroadcastTx API Node Unavailable. Could not perform query: ' +
            e.message
        )
      }
    },

    async ServiceGetTxsEvent(
      { commit, rootGetters, getters },
      {
        options: { subscribe, all } = { subscribe: false, all: false },
        params,
      ...
      try {
        ...
        while (
          ...
          let next_values = (
            await queryClient.serviceGetTxsEvent({
              ...query,
        ...
        throw new Error(
          'QueryClient:ServiceGetTxsEvent API Node Unavailable. Could not perform query: ' +
            e.message
        )
      }
    }
