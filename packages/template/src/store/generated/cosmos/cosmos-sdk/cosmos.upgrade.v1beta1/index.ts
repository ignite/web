  ...
  actions: {
    ...
    async QueryCurrentPlan(
      ...
      try {
        ...
        if (subscribe)
          commit('SUBSCRIBE', {
            action: 'QueryCurrentPlan',
            payload: { options: { all }, params: { ...key }, query }
          })
        return getters['getCurrentPlan']({ params: { ...key }, query }) ?? {}
      } catch (e) {
        throw new Error(
          'QueryClient:QueryCurrentPlan API Node Unavailable. Could not perform query: ' +
            e.message
        )
      }
    },

    async QueryAppliedPlan(
      { commit, rootGetters, getters },
      {
        options: { subscribe, all } = { subscribe: false, all: false },
        params,
      ...
      try {
        ...
        if (subscribe)
          commit('SUBSCRIBE', {
            action: 'QueryAppliedPlan',
            payload: { options: { all }, params: { ...key }, query }
          })
        return getters['getAppliedPlan']({ params: { ...key }, query }) ?? {}
      } catch (e) {
        throw new Error(
          'QueryClient:QueryAppliedPlan API Node Unavailable. Could not perform query: ' +
            e.message
        )
      }
    },
    ...
    async QueryUpgradedConsensusState(
      ...
      try {
        ...
        const queryClient = await initQueryClient(rootGetters)
        let value = (
          await queryClient.queryUpgradedConsensusState(key.last_height)
        ).data
    ...
    async QueryModuleVersions(
      ...
      try {
        ...
        while (
          ...
          let next_values = (
            await queryClient.queryModuleVersions({
              ...query,
        ...
        if (subscribe)
          commit('SUBSCRIBE', {
            action: 'QueryModuleVersions',
            payload: { options: { all }, params: { ...key }, query }
          })
        return getters['getModuleVersions']({ params: { ...key }, query }) ?? {}
      } catch (e) {
        throw new Error(
          'QueryClient:QueryModuleVersions API Node Unavailable. Could not perform query: ' +
            e.message
        )
      }
    }
