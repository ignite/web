  ...
  actions: {
    ...
    async QueryInflation(
      ...
      try {
        ...
        if (subscribe)
          commit('SUBSCRIBE', {
            action: 'QueryInflation',
            payload: { options: { all }, params: { ...key }, query }
          })
        return getters['getInflation']({ params: { ...key }, query }) ?? {}
      } catch (e) {
        throw new Error(
          'QueryClient:QueryInflation API Node Unavailable. Could not perform query: ' +
            e.message
        )
      }
    },

    async QueryAnnualProvisions(
      { commit, rootGetters, getters },
      {
        options: { subscribe, all } = { subscribe: false, all: false },
        params,
      ...
      try {
        ...
        if (subscribe)
          commit('SUBSCRIBE', {
            action: 'QueryAnnualProvisions',
            payload: { options: { all }, params: { ...key }, query }
          })
        return (
          getters['getAnnualProvisions']({ params: { ...key }, query }) ?? {}
        )
      } catch (e) {
        throw new Error(
          'QueryClient:QueryAnnualProvisions API Node Unavailable. Could not perform query: ' +
            e.message
        )
      }
    }
