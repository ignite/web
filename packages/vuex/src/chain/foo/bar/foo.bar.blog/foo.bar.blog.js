...
export default {
	...
	actions: {
		...
		async QueryPost({ commit, rootGetters }, { id, subscribe = false }) {
			try {
				const post = (await (await queryClient({addr: rootGetters['chain/common/env/apiCosmos']})).queryPost(id)).data
		...
		async QueryPostAll({ commit, rootGetters }, { subscribe = false }) {
			try {
				const post = (await (await queryClient({addr: rootGetters['chain/common/env/apiCosmos']})).queryPostAll()).data
		...
		async MsgCreatePost({rootGetters}, { value }) {
			...
			try {
				const msg= await (await txClient(rootGetters['chain/common/wallet/signer'], {addr: rootGetters['chain/common/env/apiTendermint']})).msgCreatePost(value)
		...
		async MsgUpdatePost({rootGetters}, { value }) {
			...
			try {
				const msg= await (await txClient(rootGetters['chain/common/wallet/signer'], {addr: rootGetters['chain/common/env/apiTendermint']})).msgUpdatePost(value)
				await (await txClient(rootGetters['chain/common/wallet/signer'], {addr: rootGetters['chain/common/env/apiTendermint']})).signAndBroadcast([msg])
			} catch (e) {
		...
		async MsgDeletePost({rootGetters}, { value }) {
			...
			try {
				const msg= await (await txClient(rootGetters['chain/common/wallet/signer'], {addr: rootGetters['chain/common/env/apiTendermint']})).msgDeletePost(value)
				await (await txClient(rootGetters['chain/common/wallet/signer'], {addr: rootGetters['chain/common/env/apiTendermint']})).signAndBroadcast([msg])
			} catch (e) {
				throw 'Failed to broadcast transaction: ' + e
			}
