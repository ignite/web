<template>
	<transition name="sp-fade">
		<div v-if="blockCards.length > 0" key="default" class="container">
...
export default {
	...
	props: {
		blockCards: {
			...
			validator(value) {
				return (
					value.filter(block => block.height && block.time && block.hash)
						.length === value.length
				)
			}
		}
...
.container.fade-enter-active .card:nth-last-child(1) {
	animation: firstCardSlideIn 0.5s ease-out;
}
.container.fade-enter-active .card:nth-last-child(2) {
	animation: secondCardSlideIn 0.5s ease-out;
	animation-delay: 0.25s;
}
.container.fade-enter-active .card:nth-last-child(3) {
	animation: thirdCardSlideIn 0.5s ease-out;
