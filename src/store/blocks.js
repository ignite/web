...
const mutations = {
	...
	addErrorBlock(state, { blockHeight, errLog }) {
		state.errorsQueue.push({ blockHeight, errLog })
	},
	addErrorTx(state, { blockHeight, txEncoded, errLog, txStackCallback }) {
...
const actions = {
	...
	txErrCallback: {
		root: true,
		handler({ commit, dispatch }, { blockHeight, txEncoded, errLog }) {
			commit('addErrorTx', {
				blockHeight,
				txEncoded,
				errLog,
			...
			getBlockchain({
				...
				return async () => {
					for (let i = 0; i < fmtBlockchainRes.length; i++) {
						...
						await getBlockByHeight(appEnv.RPC, prevHeader.height, errLog => {
	...
	async setBlockMeta(
		...
		blockHolder.setBlockTxs({
			...
			txErrCallback: (txEncoded, errLog) =>
				dispatch(
					'txErrCallback',
					{
	...
	async initBlockConnection({ commit, dispatch, getters, rootGetters }) {
         ...
         */
				blockHelpers
					.getBlockByHeight(appEnv.RPC, header.height, blockErrCallback)
					.then(blockMeta => {
						dispatch('setBlockMeta', {
							header,
							blockMeta,
					...
					getters.errorsQueue.forEach((errObj, index) => {
						...
						if (errBlockInStack) {
							if (errObj.txError && errObj.txError.txEncoded) {
								txHelpers
									.getDecodedTx(appEnv.API, errObj.txError.txEncoded)
									.then(txRes => {
	...
	async setHighlightedBlockMeta({ state, rootGetters }, { block }) {
		blockHelpers
			.getBlockByHeight(
				rootGetters['cosmos/env/appEnv'].RPC,
				block.data.blockMsg.height
			)
